...
* **Диаграмма вариантов использования:** <br>
        обеспечивает высокоуровневое понимание функциональности системы и ролей пользователей. Она помогает определить границы системы и основные потоки взаимодействия, гарантируя, что система решает поставленные задачи.<br>
  ![DLTDRZ~1](https://github.com/user-attachments/assets/4282cca6-6fdf-4fb9-8816-6d70598ffd7f)
...
* **ModuleVersions**
  ...
  * Ключевые поля: Version ID (PK), ModuleID (FK к Modules), VersionString, Changelog, DownloadLink, MinMagiskVersion, FileSizeMB, UploadDate.
* **Reviews**
...
## Бизнес-Слой (Business Logic Layer - BLL)

Бизнес-слой содержит основную логику приложения, координирует работу между слоем представления и слоем доступа к данным.<br>
Структура и Паттерны:
 * Service Layer Pattern: Логика сгруппирована в сервисные классы (например, UserService, ModuleService, ModerationService), каждый из которых отвечает за определенную бизнес-область.
 * Интерфейсы сервисов (например, IUserService): Определяют контракт для бизнес-операций.
 * Конкретные сервисы (например, UserService): Реализуют интерфейсы, используя репозитории для доступа к данным и инкапсулируя бизнес-правила.

Основные компоненты:
 * Пространство имен: MagiskHub.Services (или ваше фактическое, например, Magisk_DB.Services)
 * Интерфейсы: IUserService, IModuleService, IModerationService.
 * Классы: UserService, ModuleService, ModerationService.
 * Пользовательские исключения: BusinessRuleException, NotFoundException для обработки специфичных бизнес-ситуаций.

Реализация принципов ООП:
 * Инкапсуляция: Сервисы скрывают сложную логику и многошаговые операции за простыми методами. Например, UserService.RegisterAsync инкапсулирует проверку данных, хэширование пароля, поиск роли и сохранение пользователя.
 * Полиморфизм: Достигается через использование интерфейсов. Слой представления (консоль) и другие сервисы взаимодействуют с абстракциями (IUserService), что позволяет гибко изменять реализации или использовать мок-объекты для тестирования.
 * Наследование: Хотя в данном проекте наследование классов сервисов не применялось активно (предпочтение отдавалось композиции), оно используется в моделях данных (например, User мог бы быть базовым для более специализированных типов пользователей, если бы это требовалось).

<br>
##Технологический стек
    Язык программирования: C#
    Платформа: .NET (например, .NET 6/7/8)
...
Настроить строку подключения:
    ...
    Найдите строку: var connectionString = "Host=localhost;Database=Magisk;Username=postgres;Password=1234";
    Измените параметры (Host, Database, Username, Password) в соответствии с вашей конфигурацией PostgreSQL.

Убедиться, что база данных PostgreSQL запущена и доступна.

Собрать и запустить проект:

    Через Visual Studio: Открыть проект/решение и нажать "Start".
    Через .NET CLI:
    dotnet restore
    dotnet run
